...
mSet_table <- PerformPSEA(mSet, "hsa_mfn", "current", 3 , 100)

# To view the results of the pathway analysis
mSet_plot <- PlotPeaks2Paths(mSetObj = mSet_table, imgName = "peaks_to_paths_ms1_2", format = "png", dpi = 300, width = 8, labels = "default", num_annot = 10,interactive = T)
...
#  m/z__RT
mSet <- InitDataObjects("mass_table", "mummichog", FALSE) #different

# Set parameters, ppm is 4.3 here
# Only positive mode (ESI+) included
mSet <- SetPeakFormat(mSet, "colu") #different
mSet <- UpdateInstrumentParameters(mSet, 4.3, "positive", "yes", 0.02)
mSet <- SetRTincluded(mSet, "seconds")

# Read Peak table
mSet <- Read.TextData(mSet, "malaria_feature_table.csv", "colu", "disc") #ruwu/rowp/colu/colp
mSet <- SanityCheckMummichogData(mSet)
...
mSet <- FilterVariable(mSet, "none", -1, "F", 25, F) # >feature number < 5000，ignore that

# Perform data normalization
# The normailization distribution will be displaced in figure "norm_0_dpi72.png" and "snorm_0_dpi72.png" in your working directory
